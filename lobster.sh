...
EOF
    ...
    urlencode() {
        string="${1}"
        strlen=$(printf %s "$string" | wc -c)
        encoded=""
        pos=0

        while [ "$pos" -lt "$strlen" ]; do
            pos_1based=$((pos + 1))
            c=$(expr substr "$string" "$pos_1based" 1)
    ...
    extract_from_embed() {
        ...
        if [ -z "$payload" -o -z "$signature" -o -z "$difficulty" ]; then
        ...
        json_data=$(curl -s "${API_URL}/?url=${embed_link}&payload=${payload}&signature=${signature}&nonce=${nonce}")

        case "$json_data" in
            *"error"*)
                send_notification "API Error: $(printf "%s" "$json_data" | $sed -n 's/.*"error":"\([^"]*\)".*/\1/p')"
                return 1
                ;;
        esac
...
exec 1>&3 2>&4
nerds@eatmynerds:~/repos/lobster$ ^C
nerds@eatmynerds:~/repos/lobster$ shfmt -i 4 -ci -w lobster.sh
nerds@eatmynerds:~/repos/lobster$ cat lobster.sh
#!/usr/bin/env sh
